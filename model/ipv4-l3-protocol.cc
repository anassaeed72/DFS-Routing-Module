  ...
  if (!route->GetGateway ().IsEqual (Ipv4Address ("0.0.0.0")))
    {
      if (outInterface->IsUp ())
        ...
        {
          // here the interface is down so do the magic
  ...
  else 
    {
      if (outInterface->IsUp ())
        ...
        {
          // here the iterface id down so do the magic
...
// void
// Ipv4L3Protocol::MoveToNextInterface(Ptr<const Packet>p){
//   MyTag currentTag;
//   p->PeekPacketTag (currentTag);
//   uint32_t currentIndex = currentTag.GetSimpleValue() // get from tag
//   NS_LOG_UNCOND("currentIndex for tag " << currentIndex );
//   m_node->GetDevice((currentIndex+1)%m_node->GetNDevices());

// }
// This function analogous to Linux ip_forward()
int indexOfDevice(Ptr<NetDevice> netDevice,Ptr<Node> m_node){
  uint32_t numberOfDevices = m_node->GetNDevices();
  for (int i = 0; i < numberOfDevices; i++)
  {
    /* code */
    if( netDevice == m_node->GetDevice(i)){
      return i;
    }
  }
  return -1;
