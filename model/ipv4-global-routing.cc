...
{
  NS_LOG_FUNCTION ("Constructor "<<this);
...
Ipv4GlobalRouting::NotifyInterfaceUp (uint32_t i)
{
  ...
  // vectorOfDownInterfaces.erase(std::remove(vectorOfDownInterfaces.begin(), vectorOfDownInterfaces.end(), i), vectorOfDownInterfaces.end());
...
{
  NS_LOG_FUNCTION (this << i);
  vectorOfDownInterfaces.push_back(i);
  if (m_respondToInterfaceEvents && Simulator::Now ().GetSeconds () > 0)  // avoid startup events
    {
      // GlobalRouteManager::DeleteGlobalRoutes ();
      // GlobalRouteManager::BuildGlobalRoutingDatabase ();
...
Ptr<Ipv4Route>
Ipv4GlobalRouting::RouteOutputDFS (Ptr<Packet> p, const Ipv4Header &header, Ptr<NetDevice> oif, Socket::SocketErrno &sockerr)
{
...
bool 
Ipv4GlobalRouting::RouteInputDFS  (Ptr<const Packet> p, const Ipv4Header &header, Ptr<const NetDevice> idev,UnicastForwardCallback ucb, MulticastForwardCallback mcb,
                                LocalDeliverCallback lcb, ErrorCallback ecb)
{ 
  NS_LOG_LOGIC("\n\n\n\n");
  NS_LOG_FUNCTION (this << p << header << header.GetSource () << header.GetDestination () << idev << idev->GetNode()->GetId());
  // Check if input device supports IP
  NS_ASSERT (m_ipv4->GetInterfaceForDevice (idev) >= 0);
  uint32_t iif = m_ipv4->GetInterfaceForDevice (idev);
  MyTag tag(numberOfSwitches);
  ...
  bool packetTagFound = p->PeekPacketTag(tag);
  if (packetTagFound)
  {
    NS_LOG_LOGIC("packetTagFound");
  } else {
    NS_LOG_LOGIC("packetTagFound not found");
  }
  // Next, try to find a route
  if (NormalRouting(tag))
  {
    ...
    while ( NetDeviceNotWorking(outputDevice) || OutputIsParent(outputDevice,tag,idev)){
      NS_LOG_LOGIC("While Loop Iteration");
...
void
Ipv4GlobalRouting::writeToFile(std::string input){
  std::ofstream myfile;
  myfile.open ("example12345.txt");
  myfile <<"abcdefghijklmonpqrstuvwxyx  "<< input <<"\n";
  myfile.close();
}
Ptr<NetDevice>
...
bool
Ipv4GlobalRouting::OutputIsParent(Ptr<const NetDevice>netDevice, MyTag tag, Ptr<const NetDevice> idev){
  NS_LOG_FUNCTION(this);
...
bool
Ipv4GlobalRouting::NetDeviceNotWorking(Ptr<const NetDevice> netDevice){
  NS_LOG_FUNCTION(this);
...
bool
Ipv4GlobalRouting::NormalRouting(MyTag tag){
...
int
Ipv4GlobalRouting::CurrVal(MyTag tag, int index){
  return tag.GetCurrValueByIndex(index);
}
int
Ipv4GlobalRouting::ParVal(MyTag tag, int index){
...
Ptr<Ipv4Route> 
Ipv4GlobalRouting::CreateRouteFromHeader(const Ipv4Header &header) const{
  NS_LOG_FUNCTION(this);
