...
protected:
  void DoDispose (void);
  uint8_t IndexOfNetDeviceInNode(Ptr<const NetDevice> netDevice) const;
  ...
  bool DeviceOutOfRange(Ptr<const NetDevice> netDevice, int index);
  ...
  void writeToFile(std::string input);
   // These methods inherited from base class
  Ptr<Ipv4Route> RouteOutputDFS (Ptr<Packet> p, const Ipv4Header &header, Ptr<NetDevice> oif, Socket::SocketErrno &sockerr);
